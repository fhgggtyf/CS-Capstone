...
def save_to_database(data):
    ...
    cursor.execute(f'''
        CREATE TABLE IF NOT EXISTS Tarkov_Official_Forum_Posts (
    ...
    ''')    
    cursor.execute(f'''
        CREATE TABLE IF NOT EXISTS Tarkov_Official_Forum_Replies (
            author TEXT,
    ...
    cursor.execute(
        'INSERT OR REPLACE INTO Tarkov_Official_Forum_Posts (post_id, post_reply_id, post_title, post_time, author, post_link) VALUES (?, ?, ?, ?, ?, ?)',
        (
            data['post_id'],
            data['post_reply_id'],
            data['post_title'],
    ...
    cursor.executemany(
        'INSERT OR REPLACE INTO Tarkov_Official_Forum_Replies (author, reply_id, reply_time, main_text, post_id, post_link) VALUES (?, ?, ?, ?, ?, ?)',
        [
            (
                reply['author'],
                reply['reply_id'],
...
def get_thread_links(listing_url, pages):
    ...
    for i in range(int(pages)):
        page_url = f"{listing_url}page/{i+1}/"
